...
function Clock(countdown, level) {
	...
	self.update = function () {
		...
		// Level 2 specific logic (direction check)
...
window.onload = function () {
	...
	onLoadAssets(function () {
		...
		(function animloop() {
			...
			syncScreens(); // Added state-driven screen sync
		})();
...
window.syncScreens = syncScreens;
function syncScreens() {
	var s1 = document.getElementById("screen_one");
	var s2 = document.getElementById("screen_two");
	var s3 = document.getElementById("screen_three");
	var controls = document.getElementById("mobile_controls");
...
function next() {
	...
	try {
		...
		console.error("Error in next(), resetting transition lock:", e);
		transitionActive = false;
	}
}
...
function iHeartYou() {
	...
	if (levelObjects && levelObjects.length > 0) {
		for (var i = 0; i < levelObjects.length; i++) {
			if (levelObjects[i] && typeof levelObjects[i].onlyPath === "function") {
	...
	var surprise = document.querySelector(".surprise-container");
	if (surprise) {
	...
	// No replay. Fuck it.
	// After 9 seconds, SHOW END SCREEN
	setTimeout(function () {
		STAGE = 5;
		syncScreens();

		// Ensure screen three is visible
		var screenThree = document.getElementById("screen_three");
		if (screenThree) {
			screenThree.style.display = "block";
			// Important: surprise-container is inside s3
			var surprise = screenThree.querySelector(".surprise-container");
			if (surprise) {
				surprise.style.display = "block";
...
if (typeof module !== 'undefined' && module.exports) {
	module.exports = { Level, Peep, DoorKey, Door, Clock, images };
}
